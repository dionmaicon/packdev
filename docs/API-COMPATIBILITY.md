...
| `--ignore-install-scripts` | Block lifecycle scripts (`preinstall`/`install`/`postinstall`) during the sandbox's own install of the candidate version — scoped to that install step only, so the app's own `pretest`/`prebuild` hooks still run normally at test time and the verdict stays meaningful (unlike setting `npm_config_ignore_scripts` in the caller's own environment, which suppresses those hooks too and can make a genuinely broken version look like it passed — see the note below). Off by default: install scripts are load-bearing for native packages (`esbuild`, `sharp`, `better-sqlite3`), so blocking them can turn a healthy version into a false `INSTALL_FAILED`. Uses `--ignore-scripts` under npm/pnpm/Yarn Classic and `--mode=skip-build` under Yarn Berry (Berry's closest equivalent, skipping build/lifecycle scripts for that install); has no effect only when the yarn generation actually running couldn't be determined — see `IGNORE_SCRIPTS_UNSUPPORTED` below. |
...
- **A `PASSED` is only as trustworthy as `--test`.** `compat` warns on the ways this can be true without meaning anything, all best-effort heuristics over your test command/config — false negatives are expected, they only need to catch the common cases:
  ...
  - **`PASS_WITH_NO_TESTS`**: raised two ways — statically, when `--test` or the jest config sets `--passWithNoTests`; and **dynamically**, when a run actually exits `0` having reported zero tests executed regardless of what the command looked like beforehand (parsed from the runner's own output — recognizes node's built-in test runner, jest, vitest, and mocha summary formats). The dynamic check is what catches a build step silently feeding an empty suite to the runner — e.g. a caller-side `npm_config_ignore_scripts` env var (not `--ignore-install-scripts`, which is scoped to compat's own sandbox install and never touches the app's own lifecycle hooks — see above) skipping a `pretest` compile step the suite depends on, which a static pattern-match over the command string can't see coming. Either way, `PASSED` may mean the suite never actually ran.
  - **`IGNORE_SCRIPTS_UNSUPPORTED`**: `--ignore-install-scripts` was passed but the yarn generation actually running (classic vs Berry — they need different arguments, `--ignore-scripts` vs `--mode=skip-build`) couldn't be resolved, so neither could be chosen safely — the candidate's own install/build scripts ran unblocked despite the flag.
  - **Per-version `testCounts`** (`{ testsRun, testsFailed, source }`, on each `CompatVersionResult`, not a report-level caveat): a best-effort scrape of how many tests the run's own output reported, summed across every fan-out target. Absent (not zero) when no recognized runner summary could be parsed — treat `undefined` as "unknown," never as zero. Lets a consumer refuse to trust a `PASSED` that tested nothing without having to inspect raw `output` text itself.
