...
| `--ignore-install-scripts` | Block lifecycle scripts (`preinstall`/`install`/`postinstall`) during the sandbox's own install of the candidate version — scoped to that install step only, so the app's own `pretest`/`prebuild` hooks still run normally at test time and the verdict stays meaningful (unlike setting `npm_config_ignore_scripts` in the caller's own environment, which suppresses those hooks too and can make a genuinely broken version look like it passed — see the note below). Off by default: install scripts are load-bearing for native packages (`esbuild`, `sharp`, `better-sqlite3`), so blocking them can turn a healthy version into a false `INSTALL_FAILED`. Has no effect under Yarn Berry, which has no such install flag — see `IGNORE_SCRIPTS_UNSUPPORTED` below. |
...
- **A `PASSED` is only as trustworthy as `--test`.** `compat` warns on the ways this can be true without meaning anything, all best-effort heuristics over your test command/config — false negatives are expected, they only need to catch the common cases:
  ...
  - **`IGNORE_SCRIPTS_UNSUPPORTED`**: `--ignore-install-scripts` was passed but the detected package manager is Yarn Berry, which has no `--ignore-scripts` install flag — the candidate's own install scripts ran unblocked despite the flag.
