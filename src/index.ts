...
program
  ...
  .option(
    ...
    "Auto-discover fan-out consumers instead of listing them: every workspace under the " +
      ...
      "every workspace with a dependency (by name, any specifier that actually resolves " +
      "locally — workspace:, file:, link:, or a plain/semver range the target's own version " +
      "satisfies) on one of those (the wrapper pattern: a lib wraps <package>, apps depend on " +
      "the lib). Ranked by distinct symbols imported from whichever of those makes it eligible, " +
      "then by whether it passes an object into that import, capped at --top. Testing only the " +
      "owning workspace's own tests is a much weaker claim than testing what actually depends " +
      "on it.",
    false,
  )
  .option(
  ...
  .option(
    ...
    "Block lifecycle scripts (preinstall/install/postinstall) during compat's own sandbox install — the candidate version is arbitrary registry code, so a triage bot testing a proposed bump may want it inert. Scoped to the install step only: the --test phase still runs with normal npm lifecycle behavior, so the app's own pretest/prebuild hooks fire and the verdict stays meaningful (unlike setting npm_config_ignore_scripts in the caller's own environment, which would suppress those hooks too). Off by default — install scripts are load-bearing for native packages (esbuild, sharp, better-sqlite3), so blocking them can turn a healthy version into a false FAILED. Uses --ignore-scripts under npm/pnpm/Yarn Classic and --mode=skip-build under Yarn Berry (which skips build/lifecycle scripts, npm's closest equivalent); has no effect only when the yarn generation actually in use couldn't be determined — see testCommandCaveats' IGNORE_SCRIPTS_UNSUPPORTED.",
    false,
  )
  .option(
    "--seed-lockfile",
