...
exports.finishProject = finishProject;
exports.addLocalDependency = addLocalDependency;
exports.removeLocalDependency = removeLocalDependency;
exports.listLocalDependencies = listLocalDependencies;
...
async function executeInstall(packageManagerInfo) {
    const { manager } = packageManagerInfo;
    try {
        console.log(`\n🔄 Running ${manager} install...`);
        return new Promise((resolve) => {
            const child = (0, child_process_1.spawn)(manager, ["install"], {
                stdio: "inherit",
                shell: true,
            });
            child.on("close", (code) => {
...
async function promptForInstall(packageManagerInfo, autoInstall = true) {
    const { manager } = packageManagerInfo;
...
function detectDependencyType(location) {
...
function resolveLocalPath(location) {
...
async function initializeProject(configPath = ".packdev.json", autoInstall = true) {
    try {
        ...
        for (const dependency of config.dependencies) {
            ...
            if (packageJson.dependencies && packageJson.dependencies[packageName]) {
                ...
                if (!isLocalPath(packageJson.dependencies[packageName]) &&
            ...
            if (packageJson.devDependencies &&
                ...
                if (!isLocalPath(packageJson.devDependencies[packageName]) &&
            ...
            if (packageJson.peerDependencies &&
                ...
                if (!isLocalPath(packageJson.peerDependencies[packageName]) &&
...
async function finishProject(configPath = ".packdev.json", autoInstall = true) {
    try {
        // Load config
        const config = await loadConfig(configPath);
        if (!config) {
...
async function addLocalDependency(packageName, location, configPath = ".packdev.json", explicitVersion, autoInstall = true) {
    try {
        ...
        config.dependencies = config.dependencies.filter((dep) => dep.package !== packageName);
...
async function removeLocalDependency(packageName, configPath = ".packdev.json") {
    try {
        // Load config
        const config = await loadConfig(configPath);
        if (!config) {
        ...
        const dependencyIndex = config.dependencies.findIndex((dep) => dep.package === packageName);
...
async function listLocalDependencies(configPath = ".packdev.json") {
    try {
        const config = await loadConfig(configPath);
        if (!config) {
            return {
...
async function validateProject(configPath = ".packdev.json") {
    const result = {
        isValid: true,
        configExists: false,
    ...
    try {
        ...
        if (packageJson && config) {
            ...
            for (const dependency of config.dependencies) {
                if (dependency.type === "local" &&
                    ...
                    result.issues.push(`Package path does not exist: ${dependency.location}`);
                    result.isValid = false;
                }
            }
        }
        ...
        result.isValid = false;
        result.issues.push(`Error validating project: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
    return result;
}
/**
 ...
 */
async function setupGitHooks(force = false, disable = false) {
    try {
