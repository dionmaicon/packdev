...
program
    .name("packdev")
    .description("CLI tool for managing local and git package development dependencies")
    .version(package_json_1.version);
program
    .command("init")
    .description("Initialize development mode - replace package.json versions with local paths or git URLs")
    .option("-c, --config <path>", "Path to .packdev.json config file", ".packdev.json")
    .option("--no-install", "Skip running package manager install after updating package.json", false)
    .action(async (options) => {
    try {
        console.log("🚀 Initializing development mode...");
        const result = await (0, packageManager_1.initializeProject)(options.config, !options.noInstall);
        if (result.success) {
            console.log("✅ Development mode initialized successfully!");
            console.log(`📝 Replaced ${result.replacedCount} dependencies with local paths or git URLs`);
            if (result.replacedPackages.length > 0) {
                console.log("📦 Development packages:");
                result.replacedPackages.forEach((pkg) => {
                    console.log(`  - ${pkg.name}: ${pkg.originalVersion} → ${pkg.localPath}`);
                });
            }
        }
        else {
...
program
    .command("finish")
    .description("Finish development mode - restore original package.json versions")
    .option("-c, --config <path>", "Path to .packdev.json config file", ".packdev.json")
    .option("--no-install", "Skip running package manager install after updating package.json", false)
    .action(async (options) => {
    try {
        console.log("🔄 Finishing development mode...");
        const result = await (0, packageManager_1.finishProject)(options.config, !options.noInstall);
        if (result.success) {
            console.log("✅ Development mode finished successfully!");
            console.log(`📝 Restored ${result.restoredCount} dependencies to original versions`);
            if (result.restoredPackages.length > 0) {
                console.log("📦 Restored packages:");
                result.restoredPackages.forEach((pkg) => {
                    console.log(`  - ${pkg.name}: ${pkg.localPath} → ${pkg.originalVersion}`);
                });
            }
        }
        else {
...
program
    ...
    .option("-c, --config <path>", "Path to .packdev.json config file", ".packdev.json")
    .option("-o, --original-version <version>", "Override version (auto-detected from package.json if not provided)")
    .option("--no-install", "Skip running package manager install after updating package.json", false)
    .action(async (packageName, location, options) => {
    try {
        console.log(`📦 Adding dependency: ${packageName} → ${location}`);
        const result = await (0, packageManager_1.addLocalDependency)(packageName, location, options.config, options.originalVersion, !options.noInstall);
        if (result.success) {
...
program
    ...
    try {
        ...
        const result = await (0, packageManager_1.removeLocalDependency)(packageName, options.config);
        if (result.success) {
...
program
    ...
    .action(async (options) => {
    try {
        const result = await (0, packageManager_1.listLocalDependencies)(options.config);
        if (result.success) {
...
program
    ...
    .action(async (options) => {
    try {
        const result = await (0, packageManager_1.validateProject)(options.config);
        console.log("📊 Project Status:");
        console.log(`Config file: ${result.configExists ? "✅ Found" : "❌ Not found"}`);
        console.log(`Package.json: ${result.packageJsonExists ? "✅ Found" : "❌ Not found"}`);
        console.log(`Development mode: ${result.isInDevMode ? "🔧 Active" : "📦 Inactive"}`);
        if (result.configExists && result.dependencies.length > 0) {
            ...
            result.dependencies.forEach((dep) => {
                ...
                console.log(`  ${status} ${typeIcon} ${typeLabel} ${dep.package}: ${dep.version} → ${dep.location}`);
            });
        }
        if (!result.isValid) {
            console.log("\n⚠️  Issues found:");
...
program
    ...
    try {
        ...
        console.log("💡 Use 'packdev add <package> <path|git-url>' to add dependencies");
    }
    catch (error) {
...
program
    .command("setup-hooks")
    .description("Setup Git pre-commit hooks to prevent accidental development dependency commits")
    ...
    try {
        ...
        const result = await (0, packageManager_1.setupGitHooks)(options.force, options.disable);
        if (result.success) {
            if (options.disable) {
                ...
                console.log("🛡️  The hooks will now prevent commits with development dependencies");
